...
def fetch_targeted_news(country_query):
    ...
    params = {
        ...
        "sortBy": "relevancy",
        "language": "en",
        "pageSize": 35, # Aggressively upscaled cache pool to ensure 3 unique stories exist for all segments
        "apiKey": NEWS_API_KEY
    }
...
def generate_expanded_matrix_html(raw_news):
    ...
    2. Section 1: Stock Markets & Finance
       - STRIC FOCUS: Focus specifically on AI-related stocks, broader market movements, US government bonds/Treasuries, and Federal interest rate decisions.
...
if __name__ == "__main__":
    main()
Here is your completely updated `app.py` script. The matrix prompt logic has been expanded and carefully re-engineered. 

Instead of generating a blended country summary, Gemini will dynamically compile **exactly 3 important news items per country for every single subject section** (totaling 9 data points per matrix block), adhering strictly to your requested language splits.
